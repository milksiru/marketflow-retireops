...
func (a *App) snapshot() map[string]any {
	...
	return map[string]any{
		...
		"family_plan": family, "taeri_plan": map[string]any{"name": "태리", "birth": "2022-11-11", "age": "성장 중", "profile": "가족 생활 계획", "summary": "가족 계획과 생활 안정성을 함께 점검합니다.", "checkpoints": []any{}, "todo": []any{}, "sources": []any{}},
	}
}
...
func familyPlan(v map[string]int64) map[string]any {
	...
	return map[string]any{"target": fmt.Sprintf("%s ~ %s", money(low), money(high)), "summary": "가족 자산과 부채를 기준으로 주거 계획을 점검합니다.", "assets": []map[string]any{{"label": "현금/주식", "value": money(cash)}, {"label": "퇴직금", "value": money(wife + mine)}, {"label": "예적금", "value": money(savings)}, {"label": "전세 보증금", "value": money(deposit)}}, "debts": []map[string]any{{"label": "차량 대출", "value": money(car)}, {"label": "전세 대출", "value": money(loan)}}, "metrics": []map[string]any{{"label": "가용 자금", "value": money(base)}, {"label": "목표 기준", "value": money((low + high) / 2)}}, "goals": []map[string]any{{"label": "매매 준비금", "current": base, "target": low, "caption": "목표 하단 기준"}, {"label": "비상금", "current": savings, "target": 10000000, "caption": "생활 안정성 우선"}}, "steps": []string{"비상금을 먼저 확보합니다.", "부채 상환 조건을 확인합니다.", "주거 목표와 월 저축금을 함께 점검합니다."}, "monthly_saving": money(monthly), "editable": editable}
}
